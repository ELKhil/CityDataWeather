...
const URLMETEO = "http://api.openweathermap.org/data/2.5/";
const APPID = "668484f6dbe8b8f6daf28d10bf49bc08"
...
function request(callback, ville){
    ...
    xhr.onreadystatechange = function() {
        ...
        if(xhr.readyState == 4 && xhr.status == 200)
        {
            ...
            callback(myJson)
            //afficherMeteo(myJson)
        }
        console.log("golilili"+xhr.status);
        if (xhr.status == 404){
            alert("Erreur nom ville");
        }
    }
    
    xhr.open("GET", `${URLMETEO}weather?q=${ville}&units=metric&lang=fr&appid=${APPID}`, true);
    xhr.send()
}
...
function buttonClickGet(){
    ...
    request(afficherMeteo, ville)
    // requestFetch(afficherMeteo, ville)
}

//Méthode Fetch
// function requestFetch(callback, ville){
        
//     fetch(`${URLMETEO}weather?q=${ville}&units=metric&lang=fr&appid=${APPID}`)
...
function button3ClickGet(){
    ...
    for(var i=0;i<40;i++){
                ...
                var dtt =new Date(data2.list[i].dt*1000)
                var condition =false;
                    if((d.getDay()=== 6) && (d.getDay()+1 === 0) ){
                        condition=true;
                    }else if (dtt.getDay() === d.getDay()+1){
                        condition=true;
                    }
         if(condition){
 ...
 function button4ClickGet(){
    ...
    $.get(url, (data2)=>{
        ...
        for(var i=0;i<40;i++){
            ...
            var dtt =new Date(data2.list[i].dt*1000)
            var condition =false;
            console.log("aujourd'hui "+ d.getDay());
            console.log("jour de la liste "+ dtt.getDay());

            
            if((d.getDay()===5) && (dtt.getDay() === 0)){
                condition=true;
            }else if (dtt.getDay() === d.getDay()+2){
                condition=true;
            }
...
function afficheHeure(){
    ...
    let heurMinutes = document.createElement('span');
    let heureText= document.createTextNode("L'heure locale   "+(heure<10 ? `0${heure}` : `${heure}` +"h : ")+(minutes<10 ? `0${minutes}` : `${minutes}`+" min  : ")+(secondes<10 ?  `0${secondes}` : `${secondes}` +" s"));
